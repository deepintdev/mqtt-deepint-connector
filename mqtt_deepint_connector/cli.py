...
@app.command()
def run(
    mqtt_broker: str = typer.Argument(required=True, help="MQTT's broker IP")
    , mqtt_port: int = typer.Argument(required=True, help="MQTT's broker port")
    , mqtt_user: str = typer.Argument(required=True, help="MQTT's broker user")
    , mqtt_password: str = typer.Argument(required=True, help="MQTT's broker password")
    , mqtt_topics: str = typer.Argument(required=True, help="topics from data comes from")
    , deepint_auth_token: str = typer.Argument(required=True, help="Authentication token for Deep Intelligence")
    , deepint_organization_id: str = typer.Argument(required=True, help="Deep intelligence organization's id, where source is located")
    , deepint_workspace_id: str = typer.Argument(required=True, help="Deep intelligence workspace's id, where source is located")
    , deepint_source_id: str = typer.Argument(required=True, help="Deep intelligence source's id, where data will be dumped")
